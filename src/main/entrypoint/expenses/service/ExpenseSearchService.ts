...
const ExpenseSearchService = (() => {
    ...
    function findExpensesByFilters(filters: ExpenseFilters): {
        items: ExpenseSearchResponseDto[];
        total: number;
        totalPages: number;
        page: number;
    } {
        ...
        const currency = (filters?.currency || Strings.EMPTY).toUpperCase().trim();
        ...
        const filteredExpenses = ExpenseRepository.findAll()
            .filter(expense => {
                ...
                const expenseCurrency = String(expense.currency || '').toUpperCase().trim();
                const currencyOk = !currency || expenseCurrency === currency;

                const expenseDate = TimeUtil.fromUtcToDate(expense.expenseDate);
                const dateOk = isWithinDateRange(expenseDate, from, to);

                return textOk && categoryOk && currencyOk && dateOk;
            })
            .sort((a, b) => {
                const da = TimeUtil.fromUtcToDate(a.expenseDate).getTime();
                const db = TimeUtil.fromUtcToDate(b.expenseDate).getTime();
                return db - da;
            })
            .map(expense => {
    ...
    function isTextIncluded(whatEverText: string, expense: ExpenseEntity): boolean {
        return !whatEverText || [
            ...
            String(expense.currency || Strings.EMPTY).toLowerCase(),
