...
function getTableRowSliceByKey(html: string, keyPattern: RegExp): string {
    ...
    if (keyIndex < 0) {
        throw new Error(`[BCPYapeServiceMapper] Key not found for pattern: ${keyPattern}`);
    }
...
function getAmountAndCurrency(html: string): { amount: number; currency: Currency } {
    ...
    if (!match) {
        throw new Error("[BCPYapeServiceMapper] Field not matched: amount & currency");
    }
...
export const BCPYapePaymentMapper: IProofOfPaymentMapper = {
