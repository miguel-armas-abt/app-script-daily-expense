...
function getTableRowSliceByKey(html: string, keyPattern: RegExp): string {
    ...
    if (keyIndex < 0) {
        throw new Error(`[BCPCreditCardMapper] Key not found for pattern: ${keyPattern}`);
    }
...
function getAmountAndCurrency(html: string): { amount: number; currency: string } {
    ...
    if (!match) {
        throw new Error("[BCPCreditCardMapper] Field not matched: amount & currency");
    }
...
export const BCPCreditCardMapper: IProofOfPaymentMapper = {
